...
function NoteCard({ title, done, priority, id, dueDate, refreshState }) {
  ...
  return (
    <div className="flex justify-start w-80 mx-5 bg-gray-300 shadow-lg hover:shadow-xl hover:bg-blue-200 m-3 rounded-lg p-2">
      <h1 onClick={() => noteDetail()} className="text-4xl mr-2 cursor-pointer sm:text-5xl bg-gray-100 font-bold text-yellow-600 rounded-lg p-1 flex justify-center items-center">
      ...
      <div className="text-md sm:text-lg flex flex-col w-full">
        ...
        <div className="flex justify-between">
            <div>
              <h2 onClick={() => noteDetail()} className="text-md font-bold cursor-pointer text-yellow-800 sm:text-lg">{title}</h2>
            </div>
